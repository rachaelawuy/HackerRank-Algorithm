//Problem at https://www.hackerrank.com/challenges/bon-appetit/problem
...
func bonAppetit(bill []int32, k int32, b int32) {
    ...
    var rightAmount,total int32
    for i:=0; i<len(bill); i++{
        total+=bill[i]
    }
    rightAmount=(total-bill[k])/2
    temp:=b-rightAmount
    result:=strconv.FormatInt(int64(temp), 10)
    if(rightAmount == b){
        result= "Bon Appetit"
    }
    fmt.Println(result)
